...
@RestController public class ProductController {
    ...
    public Mono<Response<Product>> save(@RequestBody Product product) {
        return productService.saveProduct(product).onErrorResume(e -> Mono.fromCallable(
            () -> Response.<Product>builder().code(ResponseCode.SERVER_ERROR_CODE.getCode())
                .message(e.getMessage()).value(null).build()));
    }

    @RequestMapping(value = "/products", method = {RequestMethod.GET}, produces = {
        MediaType.APPLICATION_JSON_VALUE}) public Mono<ListResponse<Product>> getProducts() {
        return productService.getProducts();
    }

    @RequestMapping(value = "/products/price", method = {RequestMethod.GET}, produces = {
        MediaType.APPLICATION_JSON_VALUE}) public Mono<ListResponse<String>> getPrices() {
        return productService.getPrices();
    }

    @RequestMapping(value = "/product/{name}", method = {RequestMethod.GET}, produces = {
        MediaType.APPLICATION_JSON_VALUE})
    public Mono<Response<Product>> getProductsByName(@PathVariable(value = "name") String name) {
        return productService.findProductByName(name);
    }

    @RequestMapping(value = "/product", method = {RequestMethod.DELETE}, produces = {
        MediaType.APPLICATION_JSON_VALUE}, consumes = {MediaType.APPLICATION_JSON_VALUE})
    public Mono<Response<String>> deleteProductByName(
        @RequestBody ProductDeleteRequest productDeleteRequest) {
        return productService.deleteProductByName(productDeleteRequest);
    }

    @RequestMapping(value = "/products/count", method = {RequestMethod.GET}, produces = {
        MediaType.APPLICATION_JSON_VALUE}) public Mono<Response<Long>> getProductCounts() {
